...
- **Arm** (`Arm.java`, rewritten 2026-09-19): ONE Kraken X60 on CAN **40** (`rio`), a **linear** belt axis (14T HTD-5 pulley, belt anchored both ends → 0.070 m per pulley rev; gearbox ratio still H-06), **no limit switches** — zero = position at power-on, **Back** re-zeroes at the current position; soft limits [0, 0.43 m] from that zero; MotionMagicVoltage with kS + kV feedforward and **kG = 0** (horizontal, `Elevator_Static` type — not a pivot); Brake, 60 A. Gains are untuned placeholders → `docs/motionmagic-tuning.md`.
