...
- **Teleop speed:** translation scalar → **0.4** (1.9 m/s), rotation scalar 0.1 → **0.3** (0.225 rot/s ≈ 81 °/s) — `DriveConstants.kTeleopScalar` / `kTeleopRotationScalar`.
- **Elevator target cap raised by 2 ft:** 7 rot (0.84 m) → 10.39 rot (**1.25 m**, +2 ft then −8 in); sim max height 1.6 m. The auto's poke height stays at `AutoConstants.kPokeElevatorHeight` = 0.84 m (change it if the poke should use the new cap).
