...
## Commands
- `frc/` — `./gradlew build` · `./gradlew simulateJava` (GUI, keyboard joystick via simgui.json) · `SUBZERO_SIM_AUTOENABLE=teleop ./gradlew simulateJava -Pheadless` (headless, auto-enabled; `SUBZERO_SIM_VISION_TEST=1` = injected-error localization test; `SUBZERO_SIM_JOG_TEST=1` = elevator/arm 1-inch jog self-test) · `./gradlew deploy` (**humans only**, robot at 10.13.60.2)
